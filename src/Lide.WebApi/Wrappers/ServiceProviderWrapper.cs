...
public sealed class ServiceProviderWrapper : IServiceProvider, IAsyncDisposable
{
    ...
    public object GetService(Type serviceType)
    {
        ...
        if (_generatedProxies.TryGetValue(instanceObject, out object value))
        {
            return value;
        }
    ...
    {
        return useDefault
            ? (TIService)scoped.GetService(typeof(TIService)) ?? defaultReturn ?? throw new Exception($"Missing service type {typeof(TIService).Name}")
